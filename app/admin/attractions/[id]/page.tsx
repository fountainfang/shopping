        ...
        fetch(`/api/attractions/${params.id}`)
            ...
            .then(data => {
                setFormData({
                    ...
                    bookingWindow: data.bookingWindow !== undefined && data.bookingWindow !== null ? String(data.bookingWindow) : "10",
                    slotInterval: data.slotInterval !== undefined && data.slotInterval !== null ? String(data.slotInterval) : "60",
                    startTime: data.startTime || "09:00",
                    endTime: data.endTime || "18:00"
                })
                setClosedDays(Array.isArray(data.closedDays) ? data.closedDays : [])
                setLoading(false)
            })
            .catch(err => {
                console.error(err)
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const payload = {
                ...
                closedDays,
                // Setting slotsUpdatedAt to null forces immediate slot regeneration
                slotsUpdatedAt: null
            }
